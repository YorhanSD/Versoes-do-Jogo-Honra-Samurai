...
public class playerControleAtaque : MonoBehaviour
{
    ...
    { 
        ...
        if (Input.GetKeyDown(KeyCode.Q) && podeAtacar == true && atacando == false)//Quando a tecla "Q" for pressionada e o player poder atacar e não estiver atacando:
          { 
            detectaInimigo();
    ...
    public void detectaInimigo()
    {
        ...
        foreach (Collider2D inimigo in colisorInimigo)
        {
            ...
            {
              inimigo.GetComponent<inimigoIIIControleDeDano>().danoKizau(danoKizau);
            }
        }
    }
